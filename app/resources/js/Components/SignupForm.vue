<script setup>
import InputError from '@/Components/InputError.vue';
import InputLabel from '@/Components/InputLabel.vue';
import PrimaryButton from '@/Components/PrimaryButton.vue';
import TextInput from '@/Components/TextInput.vue';
import {Head, Link, useForm, usePage} from '@inertiajs/vue3';
import {useReCaptcha, VueReCaptcha} from "vue-recaptcha-v3";
import Spinner from "@/Components/Spinner.vue";

const {executeRecaptcha, recaptchaLoaded} = useReCaptcha();

const getRecaptchaToken = async () => {
        await recaptchaLoaded()

        return await executeRecaptcha('login')
    },
    inputs = [...usePage().props.app.trackingParams, ...['first_name', 'last_name', 'email']],
    formConfig = {
        first_name: '',
        last_name: '',
        email: '',
        password: '',
        password_confirmation: '',
        recaptcha_response: '',
        disclaimer: null,
        child_age: '',
        postcode: ''
    },
    query = usePage().props.query,
    props = defineProps({
        trackingQuery: {
            type: Object,
            default: null
        }
    })

let stringParams = [];
const isMomsPath = query.hasOwnProperty('mom');

if (!isMomsPath) {
    formConfig.child_age = 'nochildren'
}
inputs.forEach(inputName => {
    formConfig[inputName] = query[inputName] || '';
    if (query[inputName]) {
        stringParams.push(inputName.concat('=').concat(query[inputName]));
    }
});

const form = useForm(
        formConfig
    ),
    fillRecaptchaToken = () => {
        getRecaptchaToken().then(token => {
            form.recaptcha_response = token;
        })
    },
    submit = () => {
        form.post(route('register'), {
            onFinish: (q) => {
                if (!form.hasErrors) {
                    EfConversion(props.trackingQuery);
                    fbq('track', 'Lead');
                }
                form.reset('password', 'password_confirmation');
                fillRecaptchaToken();
            },
            preserveScroll: 'errors'
        });
    },
    queryString = stringParams.length > 0 ? '?'.concat(stringParams.join('&')) : '';

fillRecaptchaToken();

</script>

<template>
    <form @submit.prevent="submit" class="relative" :class="{'opacity-50' : form.processing}">
        <Spinner v-if="form.processing"/>
        <slot :form="form" :queryString="queryString" :isMomsPath="isMomsPath">
            <div>
                <InputLabel for="first_name" value="First Name"/>

                <TextInput
                    id="first_name"
                    type="text"
                    class="mt-1 block w-full"
                    v-model="form.first_name"
                    required
                    autofocus
                    autocomplete="first_name"
                />

                <InputError class="mt-2" :message="form.errors.first_name"/>
            </div>

            <div>
                <InputLabel for="last_name" value="Last Name"/>

                <TextInput
                    id="last_name"
                    type="text"
                    class="mt-1 block w-full"
                    v-model="form.last_name"
                    required
                    autofocus
                    autocomplete="last_name"
                />

                <InputError class="mt-2" :message="form.errors.last_name"/>
            </div>

            <div class="mt-4">
                <InputLabel for="email" value="Email"/>

                <TextInput
                    id="email"
                    type="email"
                    class="mt-1 block w-full"
                    v-model="form.email"
                    required
                    autocomplete="email"
                />

                <InputError class="mt-2" :message="form.errors.email"/>
            </div>

            <div class="mt-4">
                <InputLabel for="password" value="Password"/>

                <TextInput
                    id="password"
                    type="password"
                    class="mt-1 block w-full"
                    v-model="form.password"
                    required
                    autocomplete="new-password"
                />

                <InputError class="mt-2" :message="form.errors.password"/>
            </div>

            <div class="mt-4">
                <InputLabel for="password_confirmation" value="Confirm Password"/>

                <TextInput
                    id="password_confirmation"
                    type="password"
                    class="mt-1 block w-full"
                    v-model="form.password_confirmation"
                    required
                    autocomplete="new-password"
                />

                <InputError class="mt-2" :message="form.errors.password_confirmation"/>
            </div>

            <div class="flex items-center justify-end mt-4">
                <Link
                    :href="route('login')"
                    class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800"
                >
                    Already registered?
                </Link>

                <PrimaryButton class="ms-4" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    Register
                </PrimaryButton>
            </div>
        </slot>
    </form>
</template>
