<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight px-4">Sections for `{{category.title}}`</h2>
            <div
                class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <Search v-model="form.search"/>
                <div
                    class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <a :href="route('admin.planner_sections.create', category.id)"
                          class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                        <i class="fa-solid fa-plus w-4 mr-2"></i>
                        Create Planner Section
                    </a>
                                        ...
                                        <div :id="item.id.toString().concat('-dropdown')"
                                            ...
                                            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                ...
                                                <li v-if="item.can.edit">
                                                    <a :href="route('admin.planner_sections.edit', {plannerCategory: category.id, plannerSection:item.id})"
                                            ...
                                            <div class="py-1" v-if="item.can.delete">
                                                <DeleteItemModal
                                                    :url="route('admin.planner_sections.destroy', item.id)"
                                                    v-if="!item.deleted_at"
                                                    v-slot="slotProps"
                                                    warning-text="All Planners related to this Section will be deleted!"
                                                ...
                                                <RecoverItemModal
                                                    :url="route('admin.planner_sections.recover', item.id)" v-else
                                                    v-slot="slotProps"
...
export default {
    ...
    data() {
        return {
            form: {
                ...
                plannerCategory: this.category.id
            },
    ...
    watch: {
        form: {
            handler: throttle(function () {
                ...
                this.$inertia.replace(this.route('admin.planner_sections.index', Object.keys(query).length ? query : {remember: 'forget'}))
            }, 150),
    ...
    methods: {
        ...
        reorder(value, items) {
            axios.post(this.route('admin.planner_sections.reorder'), items)
                .catch(e => {
