<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight px-4">Product Categories</h2>
            <div
                class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <Search v-model="form.search"/>
                <div
                    class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                    <Link :href="route('admin.product_categories.create')"
                                ...
                                <td class="px-4 py-3 flex items-center justify-end"
                                    ...
                                    <div :id="item.id.toString().concat('-dropdown')"
                                        ...
                                        <div class="py-1" v-if="item.can.delete">
                                            <DeleteItemModal :url="route('admin.product_categories.destroy', item.id)" v-if="!item.deleted_at"
                                                             v-slot="slotProps"
                                                             warning-text="This Category will be unavailable for users!"
                                            ...
                                            <RecoverItemModal :url="route('admin.product_categories.recover', item.id)" v-else
                                                             v-slot="slotProps"
    ...
    watch: {
        form: {
            handler: throttle(function () {
                ...
                this.$inertia.replace(this.route('admin.product_categories.index', Object.keys(query).length ? query : {remember: 'forget'}))
            }, 150),
