<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight px-4">Topics</h2>
            <div
                    ...
                    <div class="overflow-x-auto" v-if="list.data.length > 0">
                        <table class="w-full text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3 text-center">Articles Count</th>
                            ...
                            <tr class="border-b dark:border-gray-700" v-for="item in list.data" :key="item.id">
                                ...
                                <td class="px-4 py-3 text-center">{{ item.articlesCount }}</td>
                                <td class="px-4 py-3 flex items-center justify-end"
                                    ...
                                    <div :id="item.id.toString().concat('-dropdown')"
                                        ...
                                        <div class="py-1" v-if="item.can.delete">
                                            <DeleteItemModal :url="route('admin.topics.destroy', item.id)" v-if="!item.deleted_at"
                                                             v-slot="slotProps"
                                            ...
                                            <RecoverItemModal :url="route('admin.topics.recover', item.id)" v-else
                                                             v-slot="slotProps"
    ...
    watch: {
        form: {
            handler: throttle(function () {
                ...
                this.$inertia.replace(this.route('admin.topics.index', Object.keys(query).length ? query : {remember: 'forget'}))
            }, 150),
