<template>
    <Head title="Topic Editor"/>

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Topic Editor</h2>
            ...
            <div class="w-full mx-auto px-6 lg:px-8 flex md:gap-x-6 flex-col md:flex-row flex-wrap md:flex-nowrap">
                <div class="w-full py-8">
                    ...
                    <div>
                        ...
                        <WYSIWYG v-model="form.description" entity="topic"/>
    ...
    data() {
        return {
            form: useForm({
                ...
                description: this.topic?.description || null,
                slug: this.topic?.slug || null,
                image: this.topic?.image || null,
                is_active: this.topic ? this.topic.is_active : true,
            })
        }
    },
    methods: {
        submit() {
            if (this.topic?.id) {
                this.form.patch(route('admin.topics.update', this.topic.id), {});
            } else {
