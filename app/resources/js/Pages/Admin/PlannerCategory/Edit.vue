<template>
    <Head title="Planner Category Editor"/>

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Planner Category
                    Editor</h2>
            ...
            <div class="w-full mx-auto px-6 lg:px-8 flex md:gap-x-6 flex-col md:flex-row flex-wrap md:flex-nowrap">
                <div class="w-full py-8">
                    ...
                    <div class="mb-2 flex flex-col sm:flex-row gap-y-2 sm:gap-x-2">
                        ...
                        <div class="w-1/2 sm:w-1/4">
                            <InputLabel for="as_article" value="As Article" class="mb-2"/>

                            <Toggle v-model="form.as_article"/>

                            <InputError class="mt-2" :message="form.errors.as_article"/>
                        </div>
                    ...
                    <div class="mb-2 flex flex-col sm:flex-row gap-y-2 sm:gap-x-2">
                        ...
                        <div class="w-1/2 sm:w-1/4">
                            <InputLabel for="show_image" value="Show Image" class="mb-2"/>

                            <Toggle v-model="form.show_image"/>

                            <InputError class="mt-2" :message="form.errors.show_image"/>
                        </div>

                    </div>
                    <div>
                        <InputLabel for="description" value="Explanation"/>
                        <WYSIWYG v-model="form.description" entity="planner_category"/>
                    ...
                    <div class="py-6">
                        <h2 class="text-2xl text-gray-800 dark:text-gray-200 leading-tight text-center mb-8">
                            Bottom Section</h2>
                        <WYSIWYG v-model="form.links" entity="planner_category"/>
    ...
    data() {
        return {
            form: useForm({
                ...
                color: this.category?.color || 'rgba(0, 0, 0, 0)',
                links: this.category?.links || null,
                as_article: this.category?.as_article || false,
                show_image: this.category?.as_article || false,
            })
        }
    },
    methods: {
        submit() {
            if (this.category?.id) {
                this.form.patch(route('admin.planner_categories.update', this.category.id), {});
            } else {
                this.form.post(route('admin.planner_categories.store'), {});
            }
        },
        addItem() {
