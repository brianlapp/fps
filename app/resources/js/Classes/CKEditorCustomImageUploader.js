export class CKEditorCustomImageUploader {
    ...
    _initRequest() {
        ...
        // could be different.
        xhr.open('POST', '/image/upload/path', true);
    ...
    _sendRequest(file, resolve, reject) {
        ...
        // Send the request.
        axios.post(route('admin.content.uploadImage', this.entity), data).then(response => {
            if (!response || response.error || response.data?.error) {
                const error = response?.data?.error?.message || response?.error
                return reject(error || genericErrorText);
            }
